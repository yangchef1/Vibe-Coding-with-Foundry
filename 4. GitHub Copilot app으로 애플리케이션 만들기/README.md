...
- 의견서 결과는 제목, 업체 및 신청 개요, 정책 부합성, 사업성과 위험, 종합 의견, 후속 조치가 구획된 A4 표형 서식으로 렌더링하고 "PDF 저장" 버튼에서 인쇄용 CSS와 브라우저 인쇄 대화상자를 사용
...
- 실제 BNK 금융 업무 앱처럼 상단 업무 헤더, 연결 배지, 명확한 주요 버튼, 데스크톱 입력 및 결과 2열과 모바일 1열 레이아웃 구성
- BNK Red `#D7191F`를 활성 탭, 주요 CTA 버튼, 강조 텍스트와 아이콘에 일관되게 적용하고, BNK Dark Red `#8B0304`는 버튼의 hover 및 focus 상태에 사용
- 화이트 배경과 넓은 여백을 기본으로 BNK Dark Gray는 주요 텍스트와 선, BNK Light Gray는 카드 배경과 옅은 테두리에 사용. BNK Gold `#896E4A`와 BNK Silver `#909394`는 상태, 구분선, 보조 아이콘에만 절제해 사용
...
- 두 개의 업무 탭이 표시됩니다.
- `대성정공 예시 불러오기` 버튼이 추천 입력값을 자동으로 채웁니다.
- 모의 Markdown 응답의 제목과 목록이 서식에 맞게 표시됩니다.
- 추천 버튼을 클릭하면 모의 추천 결과가 나옵니다.
- 추천 결과를 의견서 탭으로 넘길 수 있습니다.
- AI 결과를 기다리는 동안 `생성 중` 문구의 점 애니메이션이 동작하고 완료되면 사라집니다.
- 의견서 결과에서 `PDF 저장` 버튼을 누르면 앱 UI와 진행 로그를 제외한 A4 의견서 초안만 인쇄할 수 있습니다.
